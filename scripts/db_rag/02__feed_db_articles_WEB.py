...
parser.add_argument('--text_name_or_id',default="LEGITEXT000006072050")
parser.add_argument('--update_articles',action='store_true',default=True)


def main(db_path:str,text_name_or_id:str,update_articles:bool,):
    ...
    if update_articles:
        for text in db_lawtexts:
            ...
            for section in p_bar:
                ...
                is_root = section.lawtext_id = text.id
                if num_incomplete>0 or is_root:
                    ...
                    to_be_completed = database.session.query(Article.legi_id).where(
                        and_(
                            Article.legi_id.in_([a['legi_id'] for a in article_list]),
                            Article.content.is_(None))).all()
                    article_list = [a for a in article_list if (a['legi_id'],) in to_be_completed]
                            
                    for i,article_dict in enumerate(article_list):
                        article = database.session.query(Article).where(
                                Article.legi_id==article_dict['legi_id']).first()
